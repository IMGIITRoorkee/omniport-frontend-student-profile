...
export class Skill extends React.Component {
  ...
  render() {
    ...
    console.log(desc);
    const additionalCourses = (this.state.data.additionalCourses != '')? (
        <Segment basic >
        <h4>Additional Courses</h4>
        <p>{this.state.data.additionalCourses}</p>
        </Segment>
        ):(null);
    const softwarePackages = (this.state.data.softwarePackages != '')? (
        <Segment basic >
        <h4>Software Packages</h4>
        <p>{this.state.data.softwarePackages}</p>
        </Segment>
        ):(null);
    const computerLanguages = (this.state.data.computerLanguages != '')? (
        <Segment basic >
        <h4>Computer Languages</h4>
        <p>{this.state.data.computerLanguages}</p>
        </Segment>
        ):(null);
    const minorCourses = (this.state.data.minorCourses != '')? (
        <Segment basic >
        <h4>Minor Courses</h4>
        <p>{this.state.data.minorCourses }</p>
        </Segment>
        ):(null);
    const languages = (this.state.data.languages != '')? (
        <Segment basic >
        <h4>Languages</h4>
